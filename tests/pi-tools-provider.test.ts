...
describe("PiToolsProvider lifecycle", () => {
  ...
  it("applies configured bash prefix and env overrides", async () => {
    const registry = new ActionRegistry();
    registry.register(new PiToolsProvider(process.cwd(), undefined, undefined, {
      commandPrefix: "export FROM_PREFIX=prefix-value",
    ...
    const result = await registry.invoke(
      "pi.bash",
      {
        command: 'printf "%s|%s|%s" "$FROM_PREFIX" "$FROM_CONFIG" "${PI_SESSION_ID-unset}"',
      },
      baseContext,
    );
    ...
    expect(result).toMatchObject({
      ok: true,
      output: "prefix-value|config-value|unset",
    });
  });
