...
const bashToolOptions = (config?: FabricBashConfig) => {
  const injectedEnv = Object.keys(config?.env ?? {}).length > 0
    ? config?.env
    : undefined;
  return {
...
export class PiToolsProvider implements FabricProvider {
  ...
  constructor(
    ...
    bashConfig?: FabricBashConfig,
  ) {
    this.#cwd = cwd;
    this.#tools = {
      read: createReadToolDefinition(cwd),
      bash: createBashToolDefinition(cwd, bashToolOptions(bashConfig)),
