...
var loadWorkerOptions = async () => {
  if (!import.meta.url.endsWith(".ts")) return import("./chunks/options-IM6IMLWM.js");
  const sourceModulePath = "./worker/options.ts";
  return import(sourceModulePath);
};
var loadWorkerRunRecord = async () => {
  if (!import.meta.url.endsWith(".ts")) return import("./chunks/run-record-IYNFBDT4.js");
  const sourceModulePath = "./worker/run-record.ts";
  return import(sourceModulePath);
};
var loadWorkerSessionExport = async () => {
  if (!import.meta.url.endsWith(".ts")) return import("./chunks/session-export-DTTE5UBV.js");
  const sourceModulePath = "./worker/session-export.ts";
  return import(sourceModulePath);
};
var loadCompactControl = async () => {
  if (!import.meta.url.endsWith(".ts")) return import("./chunks/compact-control-UALAS7NR.js");
  const sourceModulePath = "./agents/compact-control.ts";
  return import(sourceModulePath);
};
var loadClaudeCli = async () => {
  if (!import.meta.url.endsWith(".ts")) return import("./chunks/claude-cli-5VV4LOWD.js");
  const sourceModulePath = "./agents/claude-cli.ts";
  return import(sourceModulePath);
};
var loadVedaCli = async () => {
  if (!import.meta.url.endsWith(".ts")) return import("./chunks/veda-cli-VPI4FMQE.js");
...
var assistantError = (message) => {
  ...
  const summary = unique.join(" \xB7 ") || "Pi agent reported an error";
  return `${source ? `${source}: ` : ""}${summary}`.slice(0, MAX_STDERR_CHARS);
};
var runnerLabel = (runner) => runner === "claude" ? "Claude" : runner === "veda" ? "Veda" : "Pi";
...
`);
  ...
  else piArguments.push("--no-tools");
...
${schema}`
  ...
  const childBinary = options.runner === "claude" ? options.claudeBinary : options.runner === "veda" ? options.vedaBinary : options.piBinary;
  const child = spawnCli(childBinary, childArguments, {
    ...
    env: {
      ...
      ...options.ownerIdentityId ? { PI_FABRIC_OWNER_IDENTITY_ID: options.ownerIdentityId } : {},
...
`);
  ...
  const childCompleted = exitCode === 0 && !sawAgentError && (options.runner === "pi" || options.runner === "claude" && claudeResultSeen && claudeSentInputs.length === 0 && claudeSteering.length === 0 && claudeFollowUps.length === 0 || options.runner === "veda" && vedaParsed !== void 0);
  ...
  if (record.status === "completed" && options.schemaFile) {
    try {
      ...
      if (!Value.Check(schema2, value)) {
        const errors = [...Value.Errors(schema2, value)].slice(0, 5).map((error) => error.message).join("; ");
