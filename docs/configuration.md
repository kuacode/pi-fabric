...
The nested `pi.bash` tool used inside `fabric_exec` now has its own `bash` section in `fabric.json`.

- `shellPath`: optional explicit bash executable path. Leave empty to keep Pi core's platform resolution (`~/.pi/agent/settings.json` `shellPath` first, then platform defaults).
- `commandPrefix`: optional shell setup prepended to every nested `pi.bash` command. Use it for alias loading, `source ~/.profile`, or similar one-time shell bootstrap.
- `exposeSessionEnvironment`: when `true` (default), nested `pi.bash` exposes `PI_SESSION_ID`, `PI_SESSION_FILE`, `PI_PROVIDER`, `PI_MODEL`, and `PI_REASONING_LEVEL`.
- `env`: extra environment variables injected into every nested `pi.bash` command. Values must be strings; invalid env names are ignored during normalization.

Nested `pi.bash` already inherits host `process.env`. `bash.env` adds or overrides variables on top of that inherited environment. `/fabric settings` exposes `shellPath`, `commandPrefix`, and `exposeSessionEnvironment`; edit `bash.env` directly in `fabric.json` when you need persistent project-specific variables.
...
Changing Bash settings requires `/fabric reload`.
...
- `defaultTools`: the default tool allowlist for children. Include `bash` there if spawned agents should inherit shell access by default.
