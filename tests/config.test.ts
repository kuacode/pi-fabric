...
describe("Fabric configuration", () => {
  ...
  it("normalizes bash settings and filters invalid env entries", () => {
    expect(DEFAULT_FABRIC_CONFIG.bash).toEqual({
    ...
    expect(configured.bash).toEqual({
      shellPath: "/bin/bash",
      commandPrefix: "source ~/.bashrc",
      exposeSessionEnvironment: false,
      env: { APP_ENV: "test" },
    });
    expect(normalizeFabricConfig({ bash: { shellPath: " ", commandPrefix: " " } }).bash).toEqual({
      exposeSessionEnvironment: true,
      env: {},
    });
  });
